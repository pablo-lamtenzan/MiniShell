...
/*   Updated: 2020/09/23 20:23:56 by plamtenz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool			OLD______pipe_execution(t_pipe *p, t_bst *curr, t_data *data, int cmd)
{
	char			execution_path;
	int				ac;
	char			**argv;
...
bool				OLD________execute_pipe_cmd(t_bst *curr, t_data *data)
{
		/* TO DO, 
	...
	else if (curr->next && (curr->next->operator & REDIRECTION_GREATHER \
